...
import { Button, Dropdown, Grid, Pagination, Popconfirm, Table, Typography } from "antd";
			...
			? [
					{
						...
						label: (
							...
							>
								{/*
								 * Fix: <span onClick> → <button>
								 * lint/a11y/noStaticElementInteractions — span interaktiv bo'lmasligi kerak
								 * lint/a11y/useKeyWithClickEvents    — button klaviatura eventlarini tabiiy handle qiladi
								 */}
								<button
									type="button"
									style={{
	...
	return (
		<div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
			{dataSource.map((record, index) => (
				...
				>
					{/* Row number badge */}
					<div style={{ fontSize: 11, color: "#bfbfbf", marginBottom: 8, fontWeight: 500 }}>
						#{index + 1}
					</div>
...
// ─── Helpers (outside AppTable to reduce cognitive complexity) ────────────────
...
function buildActionsColumn<T extends object>(
	crudHandlers: CRUDHandlers<T>
...
function buildFinalColumns<T extends object>(
	columns: ColumnsType<T>,
	hideActionsColumn: boolean,
	crudHandlers?: CRUDHandlers<T>
): ColumnsType<T> {
	if (!hideActionsColumn && crudHandlers) {
		return [...columns, buildActionsColumn(crudHandlers)];
	}
	return columns;
}
...
function AppTable<T extends object>({
	...
	const finalColumns = buildFinalColumns(columns, hideActionsColumn, crudHandlers);
	const resolvedPagination = buildPagination(pagination, total, isMobile);
						...
						>
							<Pagination
								...
								onChange={(page, pageSize) =>
									onChange?.(
										{
											...(resolvedPagination as TablePaginationConfig),
											current: page,
											pageSize,
										},
										{},
