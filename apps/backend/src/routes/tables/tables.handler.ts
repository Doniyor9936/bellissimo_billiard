...
import { fullTables, type createTables, type deleteTable, type getOneTable, type updateTable } from "./tables.routes";
import { db } from "@/db";
...
export const createTableHandler: AppRouteHandler<typeof createTables> = async (c) => {
    ...
    if (existing) {
        return c.json(
            { message: `Stol ${body.number} allaqachon mavjud` },
            422,
        );
    }
...
export const deleteTableHandler: AppRouteHandler<typeof deleteTable> = async (c) => {
    ...
    const activeSession = await db.query.sessions.findFirst({
        where: (s, { and, eq }) =>
            and(eq(s.table_id, id), eq(s.status, "active")),
    });

    if (activeSession) {
        return c.json(
            { message: "Faol sessiya mavjud, stolni o'chirib bo'lmaydi" },
            422,
        );
    }
