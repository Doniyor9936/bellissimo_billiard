...
import { db } from "@/db";
import { sessions, tables, shifts } from "@/db/schema";
import type {
    listSessions,
    getOneSession,
    openSession,
...
const calcSessionAmounts = (session: typeof sessions.$inferSelect) => {
    ...
    const pausedMs = session.paused_at
        ? now.getTime() - new Date(session.paused_at).getTime()
        : 0;
...
export const listSessionsHandler: AppRouteHandler<typeof listSessions> = async (c) => {
    ...
    if (table_id) conditions.push(eq(sessions.table_id, table_id));
    if (shift_id) conditions.push(eq(sessions.shift_id, shift_id));
    ...
    return c.json({ data: result, meta: { total, page, limit, total_pages: Math.ceil(total / limit) } }, 200);
};
...
export const pauseSessionHandler: AppRouteHandler<typeof pauseSession> = async (c) => {
    ...
    if (!session) return c.json({ message: "Sessiya topilmadi" }, 404);
...
export const resumeSessionHandler: AppRouteHandler<typeof resumeSession> = async (c) => {
    ...
    const pausedMs = new Date().getTime() - new Date(session.paused_at!).getTime();
...
export const updateRateHandler: AppRouteHandler<typeof updateRate> = async (c) => {
    ...
    if (!session) return c.json({ message: "Faol sessiya topilmadi" }, 404);
