...
import { db } from "@/db";
import { products, inventory_logs } from "@/db/schema";
import type {
    listProducts,
    getOneProduct,
    createProduct,
...
export const listProductsHandler: AppRouteHandler<typeof listProducts> = async (c) => {
    ...
    return c.json(
        { data, meta: { total, page, limit, total_pages: Math.ceil(total / limit) } },
        200,
    );
};
...
export const getOneProductHandler: AppRouteHandler<typeof getOneProduct> = async (c) => {
    ...
    if (!product) return c.json({ message: "Mahsulot topilmadi" }, 404);
...
export const updateProductHandler: AppRouteHandler<typeof updateProduct> = async (c) => {
    ...
    if (!updated) return c.json({ message: "Mahsulot topilmadi" }, 404);
